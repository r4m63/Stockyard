...
Описать **операционную часть** уровня хранения данных — то, что нужно для запуска и эксплуатации трёх хранилищ (PostgreSQL, Redis/KeyDB, ClickHouse) под целевой нагрузкой 10 000 CCU. Документ дополняет:

- [06. Архитектура данных](06-data.md) — *что* и *где* хранится (DDL, ключи, schema CH).
- [07. Согласованность и транзакции](07-consistency.md) — *как* пишем (TX, идемпотентность).
- [09. Наблюдаемость](09-observability.md) — *как* инструментируем (см. также §9.12 — storage-specific OTel).

Здесь — connection pools, таймауты, конфиги хранилищ, health-checks, миграции, сидинг, бэкапы и failure modes.
