...
12 разделов, каждый отвечает на один вопрос. Читай линейно (от границ системы к эксплуатации) или открывай нужный документ по теме.
...
Ключевые решения зафиксированы как самостоятельные документы — что решили и почему.
...
Архитектура описывает и то, что реализуется в семестровом MVP, и точки эволюции (для отчёта/защиты). Чтобы команда из 10 человек не пугалась объёма, разделы помечены:
...
Что **обязательно делается** в MVP — суммарно: 4 микросервиса + 2 мобильных клиента + C-драйвер + PostgreSQL + Redis + ClickHouse + базовый OpenTelemetry + Load Simulator с прохождением SLO на 10к CCU.

## Соглашения

- Диаграммы — ASCII в обычных code-fences. Раньше использовался Mermaid, потом снят — текстовые схемы читаются везде без расширений.
- Контракты API — блоками кода с комментариями.
- Числовые оценки нагрузки — расчётные, подтверждаются прогоном Load Simulator.
- Документ живой, дополняется по мере реализации.

## Status

| Поле | Значение |
|---|---|
| Версия | 0.2 (после архитектурного аудита) |
| Дата | 2026-05-09 |
