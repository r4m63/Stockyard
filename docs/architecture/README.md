...
Документация разбита на самодостаточные разделы. Каждый отвечает на один конкретный вопрос об архитектуре.
Можно читать линейно — от внешних границ системы к внутреннему устройству и эксплуатации, — или открывать отдельные документы по теме.
...
Архитектура описывает и **то, что реализуется в семестровом MVP**, и **точки эволюции** (для отчёта/защиты).
Чтобы команда из 10 человек не пугалась объёма, разделы и подразделы помечены:

- 🎯 **MVP must-have** — то, что обязательно делаем за семестр.
...
- Диаграммы — в формате **Mermaid**, рендерятся в GitHub, GitLab и Obsidian без дополнительных инструментов.
- Контракты API — блоками кода с комментариями.
- Числовые оценки нагрузки — расчётные, подтверждаются прогоном Load Simulator.
- Документ **живой**: дополняется и уточняется по мере реализации.
